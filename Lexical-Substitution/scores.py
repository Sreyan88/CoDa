...
def calc_scores(scr, sentences, original_output, original_score, mask_index):
    ...
    subst_output = raw_model(sentences.cuda())
    # print(original_score)
    # print(scr)
    prop_score = proposal_score(original_score.detach().cpu(), scr)
    substitute_validation = similarity_score(original_output, subst_output, mask_index).detach().cpu()
    # print(substitute_validation)
    # print(prop_score)
    alpha = 0.003
