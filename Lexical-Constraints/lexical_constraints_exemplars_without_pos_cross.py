...
2. The document should be on the {topic_kw}: {topic}. Here are also some examples: {exemplars}.
...
"""Write a brief document with a single sentence or multiple sentences corresponding to the following abstract description: $abs$.
Additionally, the document should have the following contraints:
1. The document should be on the {topic_kw}: {topic}. Here are also some examples: {exemplars}.
...
2. The document should have a length of {min_words}-{max_words} words."""
        ...
        for idx, row in tqdm(self.df.iterrows(), total=len(self.df), desc="Generating constraints", unit="Rows/s"):
            ...
            exemplar_list.append(rand_exemplars)
        ...
        self.df["exemplars"] = exemplar_list
        ...
        for i in range(len(sentences)):
            ...
            for act_idx, top_5_i in enumerate(top_5_indexes):
                curr_prompt = final_const_list[top_5_i]
                if len(inc_kws_list[top_5_i]) > 0 and len(exc_kws_list[top_5_i]) > 0:
                    curr_prompt = curr_prompt.format(
                        exemplars=exemplar_list[i],
                        # pos_seq=pos_sequence,
                        min_words=min_words_list[top_5_i],
                        max_words=max_words_list[top_5_i],
                        topic=topic_dict[self.ds_name],
                        topic_kw=topic_kw_dict[self.ds_name],
                        inc_kws=inc_kws_list[top_5_i],
                        exc_kws=exc_kws_list[top_5_i]
                    )
                elif len(inc_kws_list[top_5_i]) > 0:
                    curr_prompt = curr_prompt.format(
                        exemplars=exemplar_list[i],
                        # pos_seq=pos_sequence,
                        topic=topic_dict[self.ds_name],
                ...
                if "3." in curr_prompt:
                    start_count = 4
                elif "2." in curr_prompt:
                    start_count = 3
                else:
                    start_count = 1         
                for abs_concept in self.abs_con_prompts[self.df.at[i,'label']]:
                    curr_prompt = curr_prompt + os.linesep + f"{start_count}. " + abs_concept
                    start_count = start_count + 1                           
                if act_idx == 0:
                    mix_const_1.append(curr_prompt)
                else:
                    mix_const_2.append(curr_prompt)
        ...
        print(len(mix_const_1))
        print(len(mix_const_2))
