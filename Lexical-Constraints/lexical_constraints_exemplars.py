...
class ConstraintGeneration:
        ...
        with open(json_path, "r") as file:
            for line in file:
                ...
                if obj["label"] in abs_prompts:
                    abs_prompts[obj["label"]].append("The sentences " + "should not include the abstract concept " + concept.lower() + ".")
                else:
                    abs_prompts[obj["label"]] = ["The sentences " + "should not include the abstract concept " + concept.lower() + "."]               
        return abs_prompts
    ...
    def generate_constraints(self):
        ...
        prompt_inc_exc =[ 
"""Write sentences corresponding to the following abstract description: $abs$.
Additionally, the sentence should have the following contraints:
1. The sentences should have the following keywords: {inc_kws}, but should not have the following keywords : {exc_kws}.
2. The sentences should be on the {topic_kw}: {topic}. Here are some examples: {exemplars}.
3. The sentences should have a length of {min_words}-{max_words} words."""
, 
"""Write sentences with the following constraints:
1. The sentences should have the following keywords: {inc_kws}, but should not have the following keywords : {exc_kws}.
2. The sentences should be on the {topic_kw}: {topic}. Here are some examples: {exemplars}.
3. The sentences should have a length of {min_words}-{max_words} words."""
        ]
...
"""Write sentences corresponding to the following abstract description: $abs$.
Additionally, the sentence should have the following contraints:
1. The sentences should have the following keywords: {inc_kws}.
2. The sentences should be on the {topic_kw}: {topic}. Here are some examples: {exemplars}.
3. The sentences should have a length of {min_words}-{max_words} words."""
,
"""Write sentences with the following constraints:
...
"""Write sentences corresponding to the following abstract description: $abs$.
Additionally, the sentence should have the following contraints:
1. The sentences should be on the {topic_kw}: {topic}. Here are some examples: {exemplars}.
2. The sentences should have a length of {min_words}-{max_words} words."""
,
"""Write sentences with the following constraints:
1. The sentences should be on the {topic_kw}: {topic}. Here are some examples: {exemplars}.
2. The sentences should have a length of {min_words}-{max_words} words."""
                ...
                if idx == 1:
                    if inc_exc:
                        prompt = prompt.format(
                            ...
                            min_words=min_len,
                            max_words=max_len,
                            topic=topic_dict[self.ds_name],
                            topic_kw=topic_kw_dict[self.ds_name],
                            inc_kws=inc_kws,
                            exc_kws=exc_kws
                        )
                    elif inc:
