...
class ConstraintGeneration:
    def __init__(self, inp_path, json_path, out_path, out_file, debug):
        ...
        with open(json_path, "r") as file:
            for line in file:
                ...
                if obj["label"] in abs_prompts:
                    abs_prompts[obj["label"]].append("The {start} " + "should not include the abstract concept " + concept.lower() + ".")
                else:
                    abs_prompts[obj["label"]] = ["The {start} " + "should not include the abstract concept " + concept.lower() + "."]               
        return abs_prompts
...
1. The {start} should have the following keywords: {inc_kws}, but should not have the following keywords : {exc_kws}.
2. The {start} should have a part-of-speech sequence similar to: {pos_seq}.
3. The {start} should be on the topic: {topic}
4. The {start} should have a length of {min_words}-{max_words} words."""
        ]
...
"""Write {num_sent} {start} with the following constraints:
1. The {start} should have a part-of-speech sequence similar to: {pos_seq}.
2. The {start} should be on the topic: {topic}
2. The {start} should have a length of {min_words}-{max_words} words."""
        ]
...
def main():
    ...
    cg = ConstraintGeneration(args.input_path, args.json_path, out_dir, args.out_file, args.debug)  
