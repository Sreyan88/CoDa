...
class ConstraintGeneration:
    def __init__(self, inp_path, out_path, out_file, debug):
    ...
    def lexical_constraint(self, sentence, noun_list, verb_list, other_list, word_list):
        ...
        for idx, _ in enumerate(nv_final):
            if nv_final[idx] in alt_map:
                ...
                elif (alt_kw not in nv_final) and (alt_kw not in other_list):
    ...
    def generate_constraints(self):
        ...
        prompt_inc_exc =[ 
"""Write {start} corresponding to the following abstract description: $abs$.
Additionally, the sentence should have the following contraints:
1. The {start} should have the following keywords: {inc_kws}, but should not have the following keywords : {exc_kws}.
2. The {start} should have a part-of-speech sequence similar to: {pos_seq}.
3. The {start} should have a length of {min_words}-{max_words} words.
4. The {start} should {label} semantically similar to the sentence: {sent2}"""
, 
"""Write {num_sent} {start} with the following constraints:
1. The {start} should have the following keywords: {inc_kws}, but should not have the following keywords : {exc_kws}.
2. The {start} should have a part-of-speech sequence similar to: {pos_seq}.
3. The {start} should have a length of {min_words}-{max_words} words.
4. The {start} should {label} semantically similar to the sentence: {sent2}"""
...
"""Write {start} corresponding to the following abstract description: $abs$.
Additionally, the sentence should have the following contraints:
1. The {start} should have the following keywords: {inc_kws}.
2. The {start} should have a part-of-speech sequence similar to: {pos_seq}.
3. The {start} should have a length of {min_words}-{max_words} words.
4. The {start} should {label} semantically similar to the sentence: {sent2}"""
,
...
"""Write {start} corresponding to the following abstract description: $abs$.
Additionally, the sentence should have the following contraints:
1. The {start} should have a part-of-speech sequence similar to: {pos_seq}.
2. The {start} should have a length of {min_words}-{max_words} words.
3. The {start} should {label} semantically similar to the sentence: {sent2}"""
,
"""Write {num_sent} {start} with the following constraints:
1. The {start} should have a part-of-speech sequence similar to: {pos_seq}.
2. The {start} should have a length of {min_words}-{max_words} words.
3. The {start} should {label} semantically similar to the sentence: {sent2}"""
        ...
        for k in range(1,3):
            ...
            for _, row in tqdm(self.df.iterrows(), total=len(self.df), desc="Generating constraints", unit="Rows/s"):
                sent = clean_pipeline(row["text" + str(k)])

                sent2 = clean_pipeline(row["text" + str(1 if k == 2 else 2)])

                equivalency = "be" if row["label"] == "equivalent" or row["label"] == "duplicate" else "not be"
                
                abs_prompt = "Write an abstract of the following sentence in a few words:\n{}".format(sent)
                abs_const_list.append(abs_prompt)

                pos_sequence, noun_list, verb_list, other_list = self.get_spacy_data(sent)
                ...
                try:
                    inc_kws, exc_kws = self.lexical_constraint(sent, noun_list, verb_list, other_list, sent.split())
            ...
            self.df["inc_kws" + str(k)] = inc_kws_list
            self.df["exc_kws" + str(k)] = exc_kws_list
            self.df["pos_seq" + str(k)] = pos_seq_list
            # self.df["topic"] = topic_list
            self.df["min_words" + str(k)] = min_words_list
            self.df["max_words" + str(k)] = max_words_list
            self.df["num_sents" + str(k)] = num_sents_list
            self.df["abs_const" + str(k)] = abs_const_list

            self.df["final_const" + str(k)] = final_const_list
            self.df["final_topic_const" + str(k)] = final_const_topic_list

            self.df.to_csv(os.path.join(self.out_pth,self.out_file+"_sim.tsv"),sep="\t",index=False)

            model_name = "sentence-transformers/all-MiniLM-L6-v2"
            tokenizer = BertTokenizer.from_pretrained(model_name)
            model = BertModel.from_pretrained(model_name)

            sim_df = self.df[["text"+str(k)]]
            ...
            sentences = sim_df["text"+str(k)].to_list()
            ...
            for i in range(len(sentences)):
                ...
                mix_const_1.append(self.df.at[tar_sentences_idx[top_5_indexes[0]],'final_const' + str(k)])
                mix_const_2.append(self.df.at[tar_sentences_idx[top_5_indexes[1]],'final_const' + str(k)])
            ...
            self.df["mix_const_1_"+str(k)] = mix_const_1
            self.df["mix_const_2_"+str(k)] = mix_const_2
    ...
    def save_json_abstract(self):
        for k in range(1,3):
            ...
            for _, row in self.df.iterrows():
                json_entry = {
                    "instruction": row["abs_const"+ str(k)],
                    "input": "",
                    "output":  row["text"+ str(k)]
                }
                json_data.append(json_entry)

            # Write the JSON data to a file
            with open(os.path.join(self.out_pth,self.out_file+ "_" + str(k) +"_abs.json"), 'w') as json_file:
...
def main():
    ...
    cg = ConstraintGeneration(args.input_path, out_dir, args.out_file, args.debug)  
