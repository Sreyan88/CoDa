...
def get_kws_list(df, labels, k):
    ...
    for label in labels:
        ...
        # if label == "Education & Reference":
        #     print(top_k_kws_list[:k])
        top_k_kws_dict[label] = top_k_kws_list
        # print(label)
        # print(top_k_kws_list)
        # print()
        # print()
...
def main(args):
    ...
    ori_df = pd.read_csv(f"./ShortcutGrammar/data/low_res/{name}_{args.split}.tsv", sep="\t", header=0)

    labels_list = list(ori_df['label_name'].unique())
    # print(labels_list)

    top_k_kws_dict = get_kws_list(df, labels_list, k)
    # print(top_k_kws_dict.keys())
    # print()
    # print()
