...
def predictions_to_dict(preds, dataset):
    if "e" in preds[0]:
    ...
    label_names = data_utils.get_label_names(dataset)
...
def load_trees_from_predictions(output_dir, dataset=""):
    data = {k: {} for k in ("train", "dev")}
    data["dataset"] = dataset
    for k, name in (("train", f"{dataset}"), ("dev", f"{dataset}")):
        fn = Path(output_dir) / f"predictions.{name}.json"
        print(f"fn: {fn}")
        if not fn.exists():
            print(f"missing {fn}")
            continue
